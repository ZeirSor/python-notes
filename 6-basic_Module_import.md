...
- [3. 循环导入问题](#3-循环导入问题)
  - [3.1. 解决方案1](#31-解决方案1)
  - [3.2. 解决方案2](#32-解决方案2)
- [4. 内置模块 Built-in modules](#4-内置模块-built-in-modules)
  - [4.1. random](#41-random)
    - [4.1.1. 案例：年会抽奖](#411-案例年会抽奖)
  - [4.2. hashlib](#42-hashlib)
    - [4.2.1. 撞库](#421-撞库)
    - [4.2.2. 加盐](#422-加盐)
    - [4.2.3. 案例：用户注册+写入文件](#423-案例用户注册写入文件)
    - [4.2.4. 案例：用户注册+写入文件](#424-案例用户注册写入文件)
  - [4.3. 序列化相关](#43-序列化相关)
    - [4.3.1. json](#431-json)
      - [4.3.1.1. 中文](#4311-中文)
      - [4.3.1.2. 序列化支持类型](#4312-序列化支持类型)
      - [4.3.1.3. 案例：获取豆瓣电影](#4313-案例获取豆瓣电影)
      - [4.3.1.4. 案例：小网站给别人提供数据](#4314-案例小网站给别人提供数据)
  - [4.4. 时间相关](#44-时间相关)
    - [4.4.1. time](#441-time)
    - [4.4.2. datetime](#442-datetime)
    - [4.4.3. 案例：用户注册&存储文件&注册时间](#443-案例用户注册存储文件注册时间)
    - [4.4.4. 案例：日志写入到不同文件，按天来分割。](#444-案例日志写入到不同文件按天来分割)
    - [4.4.5. 时间格式的相互转换：](#445-时间格式的相互转换)
    - [4.4.6. 案例：考勤](#446-案例考勤)
  - [4.5. os](#45-os)
    - [4.5.1. 案例：用户注册](#451-案例用户注册)
    - [4.5.2. 案例：景区订票系统](#452-案例景区订票系统)
  - [4.6. sys](#46-sys)
  - [4.7. shutil](#47-shutil)
  - [4.8. logging](#48-logging)
    - [4.8.1. V1 打印日志](#481-v1-打印日志)
    - [4.8.2. V2 基本配置](#482-v2-基本配置)
    - [4.8.3. V3 自定义](#483-v3-自定义)
    - [4.8.4. 项目结构](#484-项目结构)
- [5. 第三方模块](#5-第三方模块)
  - [5.1. requests模块](#51-requests模块)
    - [5.1.1. 分析请求](#511-分析请求)
    - [5.1.2. 练习题](#512-练习题)
  - [5.2. json和jsonp](#52-json和jsonp)
  - [5.3. HTML格式](#53-html格式)
    - [5.3.1. 案例：获取编辑信息](#531-案例获取编辑信息)
    - [5.3.2. 案例：联通商品商城](#532-案例联通商品商城)
  - [5.4. 小结](#54-小结)
- [6. 虚拟环境](#6-虚拟环境)
  - [6.1. 命令行](#61-命令行)
  - [6.2. Pycharm](#62-pycharm)
  - [6.3. 常见问题](#63-常见问题)
    - [6.3.1. 我给你代码](#631-我给你代码)
    - [6.3.2. 你给我代码](#632-你给我代码)
      - [6.3.2.1. pycharm的打开](#6321-pycharm的打开)
      - [6.3.2.2. 创建虚拟环境](#6322-创建虚拟环境)
      - [6.3.2.3. 安装依赖包](#6323-安装依赖包)
...
<img src="6-basic_Module_import.assets/image-20220912144155065.png" alt="image-20220912144155065" style="zoom:50%;" />

-   包是文件夹, 也是模块。

### 2.1. `__init__.py 文件`

-   导入时，会自动打开`__init__.py`文件
-   `python`解释器运行解释`__init__.py`文件, 把`__init__.py`里的名字丢入模块的命名空间
-   导入包就是导入`__init__.py`里的名字

- 通过`__all__`变量控制`import *`的行为
- 区分模块
    - 模块是直接打开文件
    - 包是打开`__init__.py`文件
- `__init__.py`用相对导入，`.`q前面必须是包，不能是一个模块。
- 执行文件不能用相对路径！ 
### 2.2. 导入包
`from 包（文件夹） import 模块(.py文件)`
...
-   在`import`之前生成变量
-   使用`if __name__ == "__main__":`避免`print`两次
